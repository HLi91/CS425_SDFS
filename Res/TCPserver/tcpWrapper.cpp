...
namespace myTCP
{
	...
	*/
	int tcpwrapper::tcpSend(char* sendingMsg, int msgLength)
	{
		...
		if (TCPDEBUG)std::cout << "outMsg: " << sendingMsg << std::endl;
		try{
			if (send(sockfd, sendingMsg, msgLength, MSG_NOSIGNAL) == -1)
			{
				perror("send");
				delete outMsg;
				return TCPCLOSE;
			...
			for (int i = 0; i <= fdmax; i++)
			{
				if (FD_ISSET(i, &read_fds))
				{
					if (i == sockfd && socketType == LISTENER)
					{
						...
						std::thread* worker = new std::thread(&tcpwrapper::callbackHelper, this, callbackMap[new_fd]);
						threadlist.push_back(worker);
